...
export default function TermsPopup() {
    ...
    useEffect(() => {
        if (!localStorage.getItem('orbitaldome_terms_accepted')) {
            // Slight delay so it appears smoothly after loading screen fades if any
            ...
            <div className="glass-panel max-w-md w-full p-6 relative flex flex-col gap-5 animate-slideUp shadow-2xl border border-cosmos-border/50 rounded-xl overflow-hidden">
                ...
                <div className="space-y-4 text-sm text-cosmos-muted">
                    <p className="text-center font-medium text-cosmos-text">
                        Bienvenido a Orbital Dome.
                    </p>
                    <p className="text-center text-xs opacity-80 px-2">
                        Para continuar explorando el universo y utilizando nuestras herramientas astronómicas, por favor revisa y acepta nuestros términos.
                    </p>
                    ...
                    <div className="p-4 bg-black/50 rounded-lg border border-white/5 h-40 overflow-y-auto custom-scrollbar text-xs leading-relaxed space-y-4 font-mono shadow-inner">
                        <div>
                            <strong className="text-cosmos-accent mb-1 inline-block">Aviso de Privacidad:</strong>
                            <p>Respetamos tu privacidad. Esta aplicación utiliza los datos de tu ubicación geográfica únicamente en tu dispositivo para calcular posiciones astronómicas locales en tiempo real. No de guardan, procesan ni se comparten tus datos personales en servidores externos en ningún momento.</p>
                        </div>
                        <div>
                            <strong className="text-cosmos-accent mb-1 inline-block">Términos y Condiciones:</strong>
                            <p>Orbital Dome es una herramienta desarrollada con fines educativos, de divulgación astronómica y entretenimiento. A pesar de utilizar algoritmos de mecánica celeste comprobados, los datos mostrados (como la posición espacial de satélites, cuerpos celestes y eventos) pueden contener desviaciones o tener márgenes de error debido a factores externos o la naturaleza de los modelos simplificados en navegador. La información obtenida aquí no debe utilizarse para fines de navegación real, misiones críticas ni propósitos científicos oficiales.</p>
                        </div>
                    </div>
                </div>
